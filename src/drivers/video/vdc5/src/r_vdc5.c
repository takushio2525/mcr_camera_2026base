...
******************************************************************************/
static vdc5_scaling_type_t
ConvertRwProcId2SclId(const vdc5_layer_id_t layer_id);
static vdc5_graphics_type_t
ConvertRwProcId2GrId(const vdc5_layer_id_t layer_id);
...
vdc5_error_t R_VDC5_Initialize(const vdc5_channel_t ch,
  ...
  c_printf("[R_VDC5_Initialize] Enter.\n");
  ret = VDC5_OK;
#ifdef R_VDC5_CHECK_PARAMETERS
  ret = VDC5_InitializeCheckPrm(ch, param);
  c_printf("[R_VDC5_Initialize] CheckPrm ret=%d\n", ret);
  ...
  c_printf("[R_VDC5_Initialize] Midpoint ret=%d\n", ret);
  if (ret == VDC5_OK) {
    VDC5_ShrdPrmSetInitParam(ch, param);

    /* Callback function */
    if (init_func != 0) {
      c_printf("[R_VDC5_Initialize] Calling init_func...\n");
      init_func(user_num);
      c_printf("[R_VDC5_Initialize] Returned from init_func.\n");
    }

    c_printf("[R_VDC5_Initialize] Calling VDC5_Initialize internal...\n");
    VDC5_Initialize(ch, param);
    c_printf("[R_VDC5_Initialize] Returned from VDC5_Initialize internal.\n");

    /* Disable all VDC5 interrupts */
    VDC5_Int_Disable(ch);

    VDC5_ShrdPrmSetResource(ch, VDC5_RESOURCE_PANEL_CLK,
                            VDC5_RESOURCE_ST_VALID);
    if (param->lvds != NULL) {
      VDC5_ShrdPrmSetResource(ch, VDC5_RESOURCE_LVDS_CLK,
                              VDC5_RESOURCE_ST_VALID);
    }
  }
  c_printf("[R_VDC5_Initialize] Exit ret=%d\n", ret);
  return ret;
...
vdc5_error_t R_VDC5_DisplayOutput(const vdc5_channel_t ch,
  ...
  if (ret == VDC5_OK) {
    ...
    VDC5_ShrdPrmSetResource(ch, VDC5_RESOURCE_LCD_PANEL,
                            VDC5_RESOURCE_ST_VALID);
  }
  return ret;
...
/**************************************************************************/ /**
                                                                              ...
                                                                              *****************************************************************************/
vdc5_error_t
R_VDC5_VideoNoiseReduction(const vdc5_channel_t ch, const vdc5_onoff_t nr1d_on,
...
vdc5_error_t R_VDC5_ImageEnhancement(
    const vdc5_channel_t ch, const vdc5_imgimprv_id_t imgimprv_id,
    const vdc5_onoff_t shp_h_on, const vdc5_enhance_sharp_t *const sharp_param,
    const vdc5_onoff_t lti_h_on, const vdc5_enhance_lti_t *const lti_param,
  ...
  if (ret == VDC5_OK) {
    /* Resource checks */
    rsrc_state = VDC5_ShrdPrmGetLayerResource(
        ch, ((imgimprv_id == VDC5_IMG_IMPRV_0) ? VDC5_LAYER_ID_0_RD
                                               : VDC5_LAYER_ID_1_RD));
    if (rsrc_state == VDC5_RESOURCE_ST_INVALID) {
      ...
      color_space = VDC5_ShrdPrmGetColorSpaceFbRd(
          ch, ((imgimprv_id == VDC5_IMG_IMPRV_0) ? VDC5_GR_TYPE_GR0
                                                 : VDC5_GR_TYPE_GR1));
  ...
  if (ret == VDC5_OK) {
    /* Setting VDC5 registers */
    VDC5_ImageEnhancement(ch, imgimprv_id, shp_h_on, sharp_param, lti_h_on,
                          lti_param, enh_area);
  }
  return ret;
...
/**************************************************************************/ /**
                                                                              ...
                                                                              *****************************************************************************/
vdc5_error_t
R_VDC5_AlphaBlendingRect(const vdc5_channel_t ch,
                         const vdc5_layer_id_t layer_id,
      ...
      if (rsrc_state == VDC5_RESOURCE_ST_INVALID) {
        ...
        if (color_space == VDC5_COLOR_SPACE_YCBCR) {
          /* Alpha blending cannot be used
             when the format of the signal read from the frame buffer is
             YCbCr422 or YCbCr444. */
          ret = VDC5_ERR_IF_CONDITION;
        }
      }
    }
    ...
    if (rsrc_state == VDC5_RESOURCE_ST_INVALID) {
      ...
      if (color_space == VDC5_COLOR_SPACE_YCBCR) {
        /* Chroma-key processing cannot be used
           when the format of the signal read from the frame buffer is YCbCr422
           or YCbCr444. */
        ret = VDC5_ERR_IF_CONDITION;
      }
    }
...
/**************************************************************************/ /**
                                                                              ...
                                                                              *****************************************************************************/
vdc5_error_t
R_VDC5_DisplayCalibration(const vdc5_channel_t ch,
...
/**************************************************************************/ /**
                                                                              ...
                                                                              *****************************************************************************/
vdc5_error_t
R_VDC5_GammaCorrection(const vdc5_channel_t ch, const vdc5_onoff_t gam_on,
