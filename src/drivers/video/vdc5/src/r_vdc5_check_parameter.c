 ...
 * Copyright (C) 2012 - 2015 Renesas Electronics Corporation. All rights
 * reserved.
 *******************************************************************************/
/**************************************************************************/ /**
...
extern void c_printf(const char *fmt, ...);
...
static vdc5_error_t
CheckTcon_Vertical(const vdc5_lcd_tcon_timing_t *const lcd_tcon_timing);
static vdc5_error_t
CheckTcon_Horizontal(const vdc5_lcd_tcon_timing_t *const lcd_tcon_timing);
static vdc5_error_t
CheckTcon_HorizontalPol(const vdc5_lcd_tcon_timing_t *const lcd_tcon_timing);
static vdc5_error_t
CheckTcon_DataEnable(const vdc5_lcd_tcon_timing_t *const lcd_tcon_timing);
...
vdc5_error_t VDC5_InitializeCheckPrm(const vdc5_channel_t ch,
  ...
  c_printf("[VDCCheck] CheckChannel ok\n");

  if (param == NULL) {
    c_printf("[VDCCheck] param is NULL\n");
  ...
  if (param->panel_icksel >= VDC5_PANEL_ICKSEL_NUM) {
    c_printf("[VDCCheck] panel_icksel invalid\n");
    error = VDC5_ERR_PARAM_UNDEFINED;
    goto END;
  }
  if ((param->panel_icksel != VDC5_PANEL_ICKSEL_LVDS) &&
      (param->panel_icksel != VDC5_PANEL_ICKSEL_LVDS_DIV7)) {
    /* Clock frequency division ratio */
    if (param->panel_dcdr >= VDC5_PANEL_CLKDIV_NUM) {
      c_printf("[VDCCheck] panel_dcdr invalid\n");
      error = VDC5_ERR_PARAM_UNDEFINED;
      goto END;
    }
  }
  ...
  if (rsrc_state != VDC5_RESOURCE_ST_INVALID) {
    ...
    if (((inp_sel_tmp == VDC5_INPUT_SEL_VDEC) &&
         (param->panel_icksel == VDC5_PANEL_ICKSEL_IMG_DV)) ||
        ((inp_sel_tmp == VDC5_INPUT_SEL_EXT) &&
         (param->panel_icksel == VDC5_PANEL_ICKSEL_IMG))) {
      c_printf("[VDC5_InitCheck] Err 1. rsrc_state=%d\n", rsrc_state);
  ...
  if (lvds_tmp == NULL) {
    ...
    if (rsrc_state == VDC5_RESOURCE_ST_INVALID) {
      ...
      if (param->panel_icksel == VDC5_PANEL_ICKSEL_LVDS_DIV7) {
        ...
        if (ch != lvds_vdc_sel_tmp) {
          ...
          c_printf("[VDC5_InitCheck] Err 2. ch=%d, lvds_vdc_sel_tmp=%d\n", ch,
                   lvds_vdc_sel_tmp);
          error = VDC5_ERR_PARAM_CONDITION;
          goto END;
        }
    ...
    c_printf("[VDCCheck] lvds_tmp not NULL\n");
    /* The clock input to frequency divider 1 */
    if (lvds_tmp->lvds_in_clk_sel >= VDC5_LVDS_INCLK_SEL_NUM) {
      error = VDC5_ERR_PARAM_UNDEFINED;
      goto END;
    }
    /* The frequency dividing value (NIDIV and NODIV) for frequency divider 1
     * and 2 */
    if ((lvds_tmp->lvds_idiv_set >= VDC5_LVDS_NDIV_NUM) ||
        (lvds_tmp->lvds_odiv_set >= VDC5_LVDS_NDIV_NUM)) {
      error = VDC5_ERR_PARAM_UNDEFINED;
      goto END;
    }
    /* Internal parameter setting for LVDS PLL */
    if (((uint32_t)lvds_tmp->lvdspll_tst &
         (uint32_t)~VDC5_CPARA_RANGE_0X0000003F) != 0u) {
    ...
    if (param->panel_icksel != VDC5_PANEL_ICKSEL_LVDS_DIV7) {
      /* When using the clock output from the LVDS PLL as the clock for the LVDS
         output interface, both channel number should match. */
      if (ch != lvds_tmp->lvds_vdc_sel) {
        /* Channel mismatch error */
        c_printf(
            "[VDC5_InitCheck] Err 3. ch=%d, lvds_vdc_sel=%d, panel_icksel=%d\n",
            ch, lvds_tmp->lvds_vdc_sel, param->panel_icksel);
        error = VDC5_ERR_PARAM_CONDITION;
        goto END;
      }
    }
    /* The frequency dividing value (NFD) for the feedback frequency */
    if (((uint32_t)lvds_tmp->lvdspll_fd &
         (uint32_t)~VDC5_CPARA_RANGE_0X000007FF) != 0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
    if ((uint32_t)lvds_tmp->lvdspll_fd <
        (uint32_t)VDC5_CPARA_LVDSPLL_FD_LIMIT_56) {
      if ((uint32_t)lvds_tmp->lvdspll_fd <
          (uint32_t)VDC5_CPARA_LVDSPLL_FD_LIMIT_24) {
        error = VDC5_ERR_PARAM_EXCEED_RANGE;
        goto END;
      }
      if (((uint32_t)lvds_tmp->lvdspll_fd >=
           (uint32_t)VDC5_CPARA_LVDSPLL_FD_LIMIT_28) &&
          ((uint32_t)lvds_tmp->lvdspll_fd <=
           (uint32_t)VDC5_CPARA_LVDSPLL_FD_LIMIT_31)) {
        error = VDC5_ERR_PARAM_EXCEED_RANGE;
        goto END;
      }
      if (((uint32_t)lvds_tmp->lvdspll_fd >=
           (uint32_t)VDC5_CPARA_LVDSPLL_FD_LIMIT_37) &&
          ((uint32_t)lvds_tmp->lvdspll_fd <=
           (uint32_t)VDC5_CPARA_LVDSPLL_FD_LIMIT_39)) {
        error = VDC5_ERR_PARAM_EXCEED_RANGE;
        goto END;
      }
      if (((uint32_t)lvds_tmp->lvdspll_fd ==
           (uint32_t)VDC5_CPARA_LVDSPLL_FD_LIMIT_46) ||
          ((uint32_t)lvds_tmp->lvdspll_fd ==
           (uint32_t)VDC5_CPARA_LVDSPLL_FD_LIMIT_47) ||
          ((uint32_t)lvds_tmp->lvdspll_fd ==
           (uint32_t)VDC5_CPARA_LVDSPLL_FD_LIMIT_55)) {
        error = VDC5_ERR_PARAM_EXCEED_RANGE;
        goto END;
      }
    }
    /* The frequency dividing value (NRD) for the input frequency */
    if (((uint32_t)lvds_tmp->lvdspll_rd &
         (uint32_t)~VDC5_CPARA_RANGE_0X0000001F) != 0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
...
END:
  if (error != VDC5_OK) {
    c_printf("[VDC5_InitCheck] Returning error=%d\n", error);
  }
  return error;
...
vdc5_error_t VDC5_VideoInputCheckPrm(const vdc5_channel_t ch,
  ...
  if (((uint32_t)param->inp_fh50 & (uint32_t)~VDC5_CPARA_RANGE_0X000003FF) !=
      0u) {
    error = VDC5_ERR_PARAM_BIT_WIDTH;
    goto END;
  }
  /* Vsync signal 1/4fH phase timing */
  if (((uint32_t)param->inp_fh25 & (uint32_t)~VDC5_CPARA_RANGE_0X000003FF) !=
      0u) {
    error = VDC5_ERR_PARAM_BIT_WIDTH;
    goto END;
  }
  ...
  dly_tmp = param->dly;
  if (dly_tmp != NULL) {
    /* Number of lines for delaying Vsync signal and field differentiation
     * signal */
    if (((uint32_t)dly_tmp->inp_vs_dly_l &
         (uint32_t)~VDC5_CPARA_RANGE_0X00000007) != 0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
    /* Field differentiation signal delay amount */
    if (((uint32_t)dly_tmp->inp_fld_dly &
         (uint32_t)~VDC5_CPARA_RANGE_0X000000FF) != 0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
    ...
    if (((uint32_t)dly_tmp->inp_vs_dly &
         (uint32_t)~VDC5_CPARA_RANGE_0X000000FF) != 0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
    ...
    if (((uint32_t)dly_tmp->inp_hs_dly &
         (uint32_t)~VDC5_CPARA_RANGE_0X000000FF) != 0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
  ...
  if (ext_sig_tmp == NULL) {
    ...
    if ((ext_sig_tmp->inp_h_pos == VDC5_EXTIN_H_POS_YCRYCB) ||
        (ext_sig_tmp->inp_h_pos == VDC5_EXTIN_H_POS_CRYCBY)) {
...
vdc5_error_t VDC5_SyncControlCheckPrm(const vdc5_channel_t ch,
  ...
  if (param->res_vs_sel != VDC5_OFF) {
    ...
    if (((uint32_t)param->res_fv & (uint32_t)~VDC5_CPARA_RANGE_0X000007FF) !=
        0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
  ...
  if (((uint32_t)param->res_fh & (uint32_t)~VDC5_CPARA_RANGE_0X000007FF) !=
      0u) {
    error = VDC5_ERR_PARAM_BIT_WIDTH;
    goto END;
  }
  /* Vsync signal delay control */
  if (((uint32_t)param->res_vsdly & (uint32_t)~VDC5_CPARA_RANGE_0X000000FF) !=
      0u) {
    error = VDC5_ERR_PARAM_BIT_WIDTH;
    goto END;
  }
...
vdc5_error_t VDC5_DisplayOutputCheckPrm(const vdc5_channel_t ch,
  ...
  /* 1/2fH timing */
  if (((uint32_t)param->tcon_half & (uint32_t)~VDC5_CPARA_RANGE_0X000007FF) !=
      0u) {
    error = VDC5_ERR_PARAM_BIT_WIDTH;
    goto END;
  }
  /* Offset Hsync signal timing */
  if (((uint32_t)param->tcon_offset & (uint32_t)~VDC5_CPARA_RANGE_0X000007FF) !=
      0u) {
    error = VDC5_ERR_PARAM_BIT_WIDTH;
    goto END;
  }
  ...
  for (tcon_sig_index = 0; tcon_sig_index < VDC5_LCD_TCONSIG_NUM;
       tcon_sig_index++) {
...
vdc5_error_t VDC5_WriteDataControlCheckPrm(const vdc5_channel_t ch,
  ...
  if ((layer_id != VDC5_LAYER_ID_0_WR) && (layer_id != VDC5_LAYER_ID_1_WR) &&
      (layer_id != VDC5_LAYER_ID_OIR_WR)) {
  ...
  /* Number of valid lines in vertical direction output by scaling-down control
   * block */
  if (((uint32_t)scldw_rt_tmp->res_out_vw &
       (uint32_t)~VDC5_CPARA_RANGE_0X000007FF) != 0u) {
    error = VDC5_ERR_PARAM_BIT_WIDTH;
    goto END;
  }
  if (((uint32_t)scldw_rt_tmp->res_out_vw & (uint32_t)VDC5_MULTIPLE_4_MASK) !=
      0u) {
    error = VDC5_ERR_PARAM_EXCEED_RANGE;
    goto END;
  }
  /* Number of valid horizontal pixels output by scaling-down control block */
  if (((uint32_t)scldw_rt_tmp->res_out_hw &
       (uint32_t)~VDC5_CPARA_RANGE_0X000007FF) != 0u) {
    error = VDC5_ERR_PARAM_BIT_WIDTH;
    goto END;
  }
  if (((uint32_t)scldw_rt_tmp->res_out_hw & (uint32_t)VDC5_MULTIPLE_4_MASK) !=
      0u) {
    error = VDC5_ERR_PARAM_EXCEED_RANGE;
    goto END;
  }
...
/**************************************************************************/ /**
                                                                              ...
                                                                              *****************************************************************************/
vdc5_error_t
VDC5_ChangeWriteProcessCheckPrm(const vdc5_channel_t ch,
                                const vdc5_layer_id_t layer_id,
...
vdc5_error_t VDC5_ReadDataControlCheckPrm(
    const vdc5_channel_t ch, const vdc5_layer_id_t layer_id,
    const vdc5_graphics_type_t graphics_id, const vdc5_read_t *const param) {
  vdc5_error_t error;
  void *btm_base_tmp;
  vdc5_resource_state_t rsrc_state;
  ...
  case VDC5_GR_TYPE_OIR:
    ...
    if ((ch != VDC5_CHANNEL_0) &&
        (param->gr_flm_sel == VDC5_GR_FLM_SEL_DISTORTION)) {
  ...
  if (param->width_read_fb != NULL) {
    ...
    if (((uint32_t)param->width_read_fb->in_vw &
         (uint32_t)~VDC5_CPARA_RANGE_0X000007FF) != 0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
    ...
    if (((uint32_t)param->width_read_fb->in_hw &
         (uint32_t)~VDC5_CPARA_RANGE_0X000007FF) != 0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
    if ((uint32_t)param->width_read_fb->in_hw <
        (uint32_t)VDC5_CPARA_RANGE_0X00000003) {
      error = VDC5_ERR_PARAM_EXCEED_RANGE;
      goto END;
    }
  }
  ...
  if ((param->gr_format == VDC5_GR_FORMAT_YCBCR422) ||
      (param->gr_format == VDC5_GR_FORMAT_YCBCR444)) {
...
/**************************************************************************/ /**
                                                                              ...
                                                                              *****************************************************************************/
vdc5_error_t
VDC5_ChangeReadProcessCheckPrm(const vdc5_channel_t ch,
                               const vdc5_layer_id_t layer_id,
  ...
  if (param->gr_disp_sel != NULL) {
    if ((*(param->gr_disp_sel) < VDC5_DISPSEL_IGNORED) ||
        (*(param->gr_disp_sel) >= VDC5_DISPSEL_NUM)) {
      error = VDC5_ERR_PARAM_UNDEFINED;
      goto END;
    }
  }
...
vdc5_error_t VDC5_StartProcessCheckPrm(const vdc5_channel_t ch,
  ...
  if (layer_id == VDC5_LAYER_ID_ALL) {
    for (graphics_types = 0; graphics_types < VDC5_GR_TYPE_NUM;
         graphics_types++) {
    ...
    if ((*gr_disp_sel_tmp < VDC5_DISPSEL_IGNORED) ||
        (*gr_disp_sel_tmp >= VDC5_DISPSEL_NUM)) {
      error = VDC5_ERR_PARAM_UNDEFINED;
      goto END;
    }
  }
...
/**************************************************************************/ /**
                                                                              ...
                                                                              *****************************************************************************/
vdc5_error_t
VDC5_VideoNoiseReductCheckPrm(const vdc5_channel_t ch,
...
/**************************************************************************/ /**
                                                                              ...
                                                                              *****************************************************************************/
vdc5_error_t
VDC5_ImageColorMatrixCheckPrm(const vdc5_channel_t ch,
  ...
  if (param->module != VDC5_COLORMTX_IMGCNT) {
    if ((param->mtx_mode == VDC5_COLORMTX_GBR_YCBCR) ||
        (param->mtx_mode == VDC5_COLORMTX_YCBCR_YCBCR)) {
      error = VDC5_ERR_PARAM_CONDITION;
      goto END;
    }
  }
  /* Offset (DC) adjustment of Y/G, B, and R signal */
  for (colormtx_index = 0; colormtx_index < VDC5_COLORMTX_OFFST_NUM;
       colormtx_index++) {
    if (((uint32_t)param->offset[colormtx_index] &
         (uint32_t)~VDC5_CPARA_RANGE_0X000000FF) != 0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
  ...
  for (colormtx_index = 0; colormtx_index < VDC5_COLORMTX_GAIN_NUM;
       colormtx_index++) {
    if (((uint32_t)param->gain[colormtx_index] &
         (uint32_t)~VDC5_CPARA_RANGE_0X000007FF) != 0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
...
/**************************************************************************/ /**
                                                                              ...
                                                                              *****************************************************************************/
vdc5_error_t
VDC5_ImageEnhancementCheckPrm(const vdc5_channel_t ch,
  ...
  if (sharp_param != NULL) {
    ...
    if (((uint32_t)sharp_param->hrz_sharp[VDC5_IMGENH_SHARP_H1].shp_core &
         (uint32_t)~VDC5_CPARA_RANGE_0X0000007F) != 0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
    if (((uint32_t)sharp_param->hrz_sharp[VDC5_IMGENH_SHARP_H2].shp_core &
         (uint32_t)~VDC5_CPARA_RANGE_0X0000007F) != 0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
    if (((uint32_t)sharp_param->hrz_sharp[VDC5_IMGENH_SHARP_H3].shp_core &
         (uint32_t)~VDC5_CPARA_RANGE_0X0000007F) != 0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
  ...
  if (enh_area != NULL) {
    ...
    if (((uint32_t)enh_area->vs & (uint32_t)~VDC5_CPARA_RANGE_0X000007FF) !=
        0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
    ...
    if (((uint32_t)enh_area->vw & (uint32_t)~VDC5_CPARA_RANGE_0X000007FF) !=
        0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
    /* Horizontal start position */
    if (((uint32_t)enh_area->hs & (uint32_t)~VDC5_CPARA_RANGE_0X000007FF) !=
        0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
    ...
    /* Horizontal width */
    if (((uint32_t)enh_area->hw & (uint32_t)~VDC5_CPARA_RANGE_0X000007FF) !=
        0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
...
/**************************************************************************/ /**
                                                                              ...
                                                                              *****************************************************************************/
vdc5_error_t
VDC5_ImageBlackStretchCheckPrm(const vdc5_channel_t ch,
                               const vdc5_imgimprv_id_t imgimprv_id,
  ...
  if (param != NULL) {
    ...
    if (((uint32_t)param->bkstr_st & (uint32_t)~VDC5_CPARA_RANGE_0X0000000F) !=
        0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
    /* Black stretch depth */
    if (((uint32_t)param->bkstr_d & (uint32_t)~VDC5_CPARA_RANGE_0X0000000F) !=
        0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
    /* Black stretch time constant (T1) */
    if (((uint32_t)param->bkstr_t1 & (uint32_t)~VDC5_CPARA_RANGE_0X0000001F) !=
        0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
    /* Black stretch time constant (T2) */
    if (((uint32_t)param->bkstr_t2 & (uint32_t)~VDC5_CPARA_RANGE_0X0000001F) !=
        0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
...
/**************************************************************************/ /**
                                                                              ...
                                                                              *****************************************************************************/
vdc5_error_t
VDC5_AlphaBlendingCheckPrm(const vdc5_channel_t ch,
                           const vdc5_layer_id_t layer_id,
  ...
  if ((layer_id != VDC5_LAYER_ID_1_RD) && (layer_id != VDC5_LAYER_ID_2_RD) &&
...
/**************************************************************************/ /**
                                                                              ...
                                                                              *****************************************************************************/
vdc5_error_t
VDC5_AlphaBlendingRectCheckPrm(const vdc5_channel_t ch,
                               const vdc5_layer_id_t layer_id,
  ...
  if ((layer_id != VDC5_LAYER_ID_1_RD) && (layer_id != VDC5_LAYER_ID_2_RD) &&
      (layer_id != VDC5_LAYER_ID_3_RD) && (layer_id != VDC5_LAYER_ID_VIN_RD)) {
    error = VDC5_ERR_PARAM_LAYER_ID;
    goto END;
  }
  ...
  if (param != NULL) {
    ...
    if (param->gr_arc != NULL) {
      /* Vertical start position */
      if (((uint32_t)param->gr_arc->vs_rel &
           (uint32_t)~VDC5_CPARA_RANGE_0X000007FF) != 0u) {
        error = VDC5_ERR_PARAM_BIT_WIDTH;
        goto END;
      }
      /* Height (vertical width) */
      if (((uint32_t)param->gr_arc->vw_rel &
           (uint32_t)~VDC5_CPARA_RANGE_0X000007FF) != 0u) {
        error = VDC5_ERR_PARAM_BIT_WIDTH;
        goto END;
      }
      /* Horizontal start position */
      if (((uint32_t)param->gr_arc->hs_rel &
           (uint32_t)~VDC5_CPARA_RANGE_0X000007FF) != 0u) {
        error = VDC5_ERR_PARAM_BIT_WIDTH;
        goto END;
      }
      /* Horizontal width */
      if (((uint32_t)param->gr_arc->hw_rel &
           (uint32_t)~VDC5_CPARA_RANGE_0X000007FF) != 0u) {
        error = VDC5_ERR_PARAM_BIT_WIDTH;
        goto END;
      }
    ...
    if (param->alpha_rect != NULL) {
      ...
      if ((param->alpha_rect->gr_arc_coef <
           (int16_t)VDC5_CPARA_GR_ARC_COEF_MIN) ||
          (param->alpha_rect->gr_arc_coef >
           (int16_t)VDC5_CPARA_GR_ARC_COEF_MAX)) {
        error = VDC5_ERR_PARAM_EXCEED_RANGE;
        goto END;
      }
    }
...
vdc5_error_t VDC5_ChromakeyCheckPrm(const vdc5_channel_t ch,
  ...
  if (param != NULL) {
    ...
    /* The following four cases are intentionally combined,
      since the four types of graphics format are represented as 16 bit unsigned
      integer. */
...
/**************************************************************************/ /**
                                                                              ...
                                                                              *****************************************************************************/
vdc5_error_t
VDC5_DisplayCalibrationCheckPrm(const vdc5_channel_t ch,
  ...
  if (bright_tmp != NULL) {
    ...
    if (((uint32_t)bright_tmp->pbrt_g &
         (uint32_t)~VDC5_CPARA_RANGE_0X000003FF) != 0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
    /* Brightness (DC) adjustment of B signal */
    if (((uint32_t)bright_tmp->pbrt_b &
         (uint32_t)~VDC5_CPARA_RANGE_0X000003FF) != 0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
    /* Brightness (DC) adjustment of R signal */
    if (((uint32_t)bright_tmp->pbrt_r &
         (uint32_t)~VDC5_CPARA_RANGE_0X000003FF) != 0u) {
      error = VDC5_ERR_PARAM_BIT_WIDTH;
      goto END;
    }
  ...
  if (panel_dither_tmp != NULL) {
    ...
    if (panel_dither_tmp->pdth_sel == VDC5_PDTH_MD_2X2) {
      ...
      if (((uint32_t)panel_dither_tmp->pdth_pa &
           (uint32_t)~VDC5_CPARA_RANGE_0X00000003) != 0u) {
        error = VDC5_ERR_PARAM_BIT_WIDTH;
        goto END;
      }
      if (((uint32_t)panel_dither_tmp->pdth_pb &
           (uint32_t)~VDC5_CPARA_RANGE_0X00000003) != 0u) {
        error = VDC5_ERR_PARAM_BIT_WIDTH;
        goto END;
      }
      if (((uint32_t)panel_dither_tmp->pdth_pc &
           (uint32_t)~VDC5_CPARA_RANGE_0X00000003) != 0u) {
        error = VDC5_ERR_PARAM_BIT_WIDTH;
        goto END;
      }
      if (((uint32_t)panel_dither_tmp->pdth_pd &
           (uint32_t)~VDC5_CPARA_RANGE_0X00000003) != 0u) {
        error = VDC5_ERR_PARAM_BIT_WIDTH;
        goto END;
      }
...
/**************************************************************************/ /**
                                                                              ...
                                                                              *****************************************************************************/
vdc5_error_t
VDC5_GammaCorrectionCheckPrm(const vdc5_channel_t ch,
...
static vdc5_error_t CheckRect(const vdc5_period_rect_t *const rect) {
  ...
  if (((uint32_t)rect->vs + (uint32_t)rect->vw) >
      (uint32_t)VDC5_CPARA_VS_VW_MAX) {
    error = VDC5_ERR_PARAM_EXCEED_RANGE;
    goto END;
  }
  ...
  if (((uint32_t)rect->hs + (uint32_t)rect->hw) >
      (uint32_t)VDC5_CPARA_HS_HW_MAX) {
    error = VDC5_ERR_PARAM_EXCEED_RANGE;
    goto END;
  }
END:
...
CheckTcon_Horizontal(const vdc5_lcd_tcon_timing_t *const lcd_tcon_timing) {
  ...
  if (((uint32_t)lcd_tcon_timing->tcon_hsvs &
       (uint32_t)~VDC5_CPARA_RANGE_0X000007FF) != 0u) {
    error = VDC5_ERR_PARAM_BIT_WIDTH;
    goto END;
  }
  /* Pulse width */
  if (((uint32_t)lcd_tcon_timing->tcon_hwvw &
       (uint32_t)~VDC5_CPARA_RANGE_0X000007FF) != 0u) {
    error = VDC5_ERR_PARAM_BIT_WIDTH;
    goto END;
  }
  /* LCD TCON output pin select */
  if ((lcd_tcon_timing->tcon_pin < VDC5_LCD_TCON_PIN_NON) ||
      (lcd_tcon_timing->tcon_pin >= VDC5_LCD_TCON_PIN_NUM)) {
    error = VDC5_ERR_PARAM_UNDEFINED;
    goto END;
  }
END:
...
static vdc5_error_t CheckGammaCrct_GainAdjustment(const uint16_t *gam_gain) {
  ...
  if (gam_gain != NULL) {
    for (gain_index = 0; gain_index < VDC5_GAM_GAIN_ADJ_NUM; gain_index++) {
      if (((uint32_t)*gam_gain & (uint32_t)~VDC5_CPARA_RANGE_0X000007FF) !=
          0u) {
        error = VDC5_ERR_PARAM_BIT_WIDTH;
        goto END;
      }
