...
static void init_func(const uint32_t user_num) {
  ...
  if ((vdc5_channel_t)user_num == VDC5_CHANNEL_0) {
    ...
    /* Standby Request Register 2 (STBREQ2)
        b5      --0-----;  STBRQ25 : The standby request to VDC5 channel 0 is
       invalid. */
    ...
    /* Standby control register 9 (STBCR9)
        b0      -------0;  MSTP90 : 0 : Video display controller channel 1
       enable */
    ...
    /* Standby Request Register 2 (STBREQ2)
        b4      ---0----;  STBRQ24 : The standby request to VDC5 channel 1 is
       invalid. */
...
drv_graphics_error_t DRV_Graphics_Init(drv_lcd_config_t *drv_lcd_config) {
  ...
  uint32_t LvdsUsed = LVDS_IF_NOT_USE;
  ...
  if (drv_error == DRV_GRAPHICS_OK) {
    ...
    /* Sync signal control */
    sync_ctrl.res_vs_sel = VDC5_ON; /* Vsync signal output select (free-running
                                       Vsync on/off control) */
    /* Sync signal output and full-screen enable signal select */
    sync_ctrl.res_vs_in_sel = VDC5_RES_VS_IN_SEL_SC0;
    sync_ctrl.res_fv = drv_lcd_config->v_toatal_period -
                       1; /* Free-running Vsync period setting */
    sync_ctrl.res_fh =
        drv_lcd_config->h_toatal_period - 1; /* Hsync period setting */
  ...
  if (drv_error == DRV_GRAPHICS_OK) {
    ...
    /* Output parameter */
    output.tcon_half = (drv_lcd_config->h_toatal_period - 1) /
                       2; /* TCON reference timing, 1/2fH timing */
    output.tcon_offset =
    ...
    if (drv_lcd_config->de_port != DRV_LCD_TCON_PIN_NON) {
      ...
      output.outctrl[VDC5_LCD_TCONSIG_STVB_VE] =
          &lcd_tcon_timing_VE; /* STVB/VE: Not used   */
      ...
      output.outctrl[VDC5_LCD_TCONSIG_STB_LP_HE] =
          &lcd_tcon_timing_HE; /* STB/LP/HE: Not used */
    ...
    output.out_frq_sel =
        VDC5_LCD_PARALLEL_CLKFRQ_1; /* Clock frequency control */
    output.out_dir_sel =
        VDC5_LCD_SERIAL_SCAN_FORWARD;              /* Scan direction select */
    output.out_phase = VDC5_LCD_SERIAL_CLKPHASE_0; /* Clock phase adjustment */
    output.bg_color =
        (uint32_t)0x00000000u; /* Background color in 24-bit RGB color format */
    /* Display output */
  ...
  c_printf("[DRV_Graphics_Init] End result: %d\n", drv_error);
  return drv_error;
...
DRV_Graphics_Video_init(drv_video_input_sel_t drv_video_input_sel,
  ...
  input.inp_fh50 =
      (uint16_t)VSYNC_1_2_FH_TIMING; /* Vsync signal 1/2fH phase timing */
  input.inp_fh25 =
...
/**************************************************************************/ /**
                                                                              ...
                                                                              ******************************************************************************/
drv_graphics_error_t
DRV_Video_Write_Setting(drv_video_input_channel_t video_input_ch,
  ...
  if (drv_error == DRV_GRAPHICS_OK) {
    ...
    scldw_rot->res_pfil_sel =
        VDC5_ON; /* Prefilter mode select for brightness signals (on/off) */
    scldw_rot->res_out_vw =
